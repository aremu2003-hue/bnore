...
function Services() {
    ...
    const firstServiceId = servicesList[0]?.id || 'personal-care';

    // Redirect to first service if no serviceId is provided
    if (!serviceId) {
        return <Navigate to={`/services/${firstServiceId}`} replace />;
    }

    // Redirect if serviceId doesn't exist in servicesData
    if (serviceId && !servicesData[serviceId]) {
        return <Navigate to={`/services/${firstServiceId}`} replace />;
    }

    const [activeService, setActiveService] = useState(serviceId || firstServiceId);
    ...
    useEffect(() => {
        // Set active service when serviceId changes
        if (serviceId && servicesData[serviceId]) {
                ...
                <div className="services-hero-container">
                    <div className="services-hero-content">
                        <div className="services-hero-text" data-aos="fade-up">
                            <div className="services-badge1">{servicesCopy.hero.badge}</div>
                ...
                <div className="service-details-container">
                    <div className="service-content" data-aos="fade-up">
                        <div className="service-header">
                            ...
                            {/* Real Talk Section */}
                            {currentService.realTalk && (
                                <div className="service-real-talk" data-aos="fade-up" data-aos-delay="50">
                                    <span className="real-talk-label">Real Talk:</span>
                                    <p className="real-talk-content">{currentService.realTalk}</p>
                                </div>
                            )}
                        </div>
                            ...
                            {servicesCopy.contact.contactInfo.map((item, index) => (
                                <div key={index} className="contact-info-item" data-aos="fade-up" data-aos-delay={200 + (index * 200)}>
                                    ...
                                    {item.title === "Call Us" || item.title === "Email Us" ? (
                                        <a href={item.title === "Call Us" ? "tel:7133702755" : "mailto:info@cyrilliahomehealth.com"} className="contact-link">{item.details}</a>
                                    ) : (
